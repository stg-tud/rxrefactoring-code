...
import oracle9_ForkJoin.FibonacciRx;
...
public class Tests
{
    ...
    public void testFibonacciRx(Long input, Long expected) throws InterruptedException
    {
        long startTime = System.currentTimeMillis();
        long[] result = new long[1];
        FibonacciRx fibonacci = new FibonacciRx(input, result);
        fibonacci.compute();
        assertEquals(expected, Optional.ofNullable(fibonacci.getResult()).orElse(-1L));
        long delta = System.currentTimeMillis() - startTime;
        ConcurrentUtils.message("Fibonacci: " + input + " took " + delta + " milliseconds");
    }
    ...
    @Test(expected = RuntimeException.class) // instead of TimeoutException when using Rx

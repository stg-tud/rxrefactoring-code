package de.tudarmstadt.refactoringrx.core;
...
import de.tudarmstadt.refactoringrx.core.processors.AbstractRefactoringProcessor;
import de.tudarmstadt.refactoringrx.core.processors.RefactoringProcessor;
...
public class RxJavaRefactoringApp extends AbstractRxJavaRefactoringApp {
	...
	public void refactorCompilationUnits(IJavaProject project, Map<String, ICompilationUnit> units ) {
		...
		Collector collector;
		try	{
			collector = this.extension.createCollector(project);
			if ( collector == null ) {
		...
		try
		{
			...
			RxLogger.error( this, "METHOD=refactorCompilationUnits, Project:" + project.getPath() + " - FAILED", e );
		}
		try	{
			latch.await();
	...
	private void refactorUnits( Collector collector ) {
		RxLogger.info( this, "METHOD=refactorUnits - " + collector.getName() + " Refactoring starting..." );

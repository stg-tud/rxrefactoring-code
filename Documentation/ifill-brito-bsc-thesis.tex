...
\code{SwingWorker}s and \code{AsyncTask}s also have some similar methods. We match these methods in Table~\ref{table:worker-task-equivalent}. Methods written in {\bfseries bold} depend on the state of the \code{SwingWorker} and/or \code{AsyncTask}.
...
\begin{itemize}
	\item {\bfseries Line 10}: we specify the thread in which the \code{Observable} operates. The scheduler \code{Schedulers.computation} corresponds to a background thread.
	\item {\bfseries Line 11}: the \code{publish} invocation uses the type \code{List<Document>}. Since it is more comfortable to work directly with \code{Document}s, rather than a list of this type, we use \code{flapMap} to extract the \code{Document} objects from the lists received from \code{publish}.
	\item {\bfseries Line 12}: at this point we have a stream of \code{Document}s. We filter these \code{Document}s to consider only jpg files.
	...
	\item {\bfseries Line 21 - 24}: we register a \code{Subscriber} using lambda notation that updates the UI and prints the name of the file. Listing~\ref{code:example-console} shows the console output. One can see that the \code{Subscriber} 0 prints the following text to the console for each file: \code{ Updating UI: <FileName>}
	\item {\bfseries Line 25 - 27}: we register further \code{Subscriber}s using lambda notation as well. Each of these \code{Subscriber}s prints the name of the file to the console. Listing~\ref{code:example-console} shows the console output for \code{Subscriber}s 1 to 3. Notice that the \code{Subscriber}s could perform more complex operations. We used the method \code{print} (Listing~\ref{code:example-edited}~Line:~34) to keep the example simple.
	\item {\bfseries Line 29}: we connect the \code{ConnectableObservable} to start the emissions.
\end{itemize}
...
\begin{itemize}
	\item {\bfseries Line 1}: We create the \code{Observable} from an \code{Emitter} of type \code{SWPackage<Integer,String>>}.
	\item {\bfseries Lines 4-17}: We define the algorithm to generate the items. The algorithm consists of iterating from zero to ten and sending a \code{String} to the \code{Subscriber} on each iteration. The indexes \code{i} used for the iteration are accumulated in the variable \code{accum} and returned at the end of the operation as \code{Integer}.
	\begin{itemize}
		\item {\bfseries Line 7}: We create the \code{SWPackage} instance that is used to generate the next item in the current iteration.
		\item {\bfseries Line 8}: We create the \code{String} value for the next item based on the iteration index \code{i}.
		\item {\bfseries Line 9}: We add the intermediate result \code{text} to the \code{SWPackage} by using the method \code{setChunks}. This method accepts multiple arguments of type \code{String} and returns a \code{SWPackage}. After that, we invoke the \code{onNext} method of the \code{emitter} using the \code{SWPackage} as argument to finally generate the item.
		\item {\bfseries Line 10}: We update the \code{accum} variable.
		\item {\bfseries Line 12}: Once the iterations has completed, we create another \code{SWPackage} to generate the item that contains the final result.
		\item {\bfseries Line 13}: Similar to Line 9, we put the result of the operation into the \code{SWPackage}. This time, we use \code{setResult} which accepts a single argument of type \code{Integer} and returns a \code{SWPackage} as well. The final result item is then generated by calling \code{onNext} on the \code{emitter} using the \code{SWPackage}.
		\item {\bfseries Lines 14-17}: Same as in Listing~\ref{code:emitter-example}
		\end{itemize}
	\item {\bfseries Lines 20-34}: We implemented the methods of the \code{Subscriber} responsible for handling the events \code{onNext}, \code{onCompleted}, \code{onError}.
		\begin{itemize}
			\item {\bfseries Lines 23}: We retrieve the intermediate results from the \code{item} using \code{getChunks}.
			\item {\bfseries Lines 24-27}: If there are intermediate results, we take the first one and print it to the console. Notice that this value corresponds to a \code{String}.
			\item {\bfseries Line 28}: We retrieve the final result from the \code{item} using \code{getResult}.
			\item {\bfseries Lines 29-31}: If there is a result, then we print it to the console. Notice that the data type of this value is \code{Integer}.
...
\begin{itemize}
	\item {\bfseries Line 9} (\code{subscribeOn}): to specify in which thread the operations should be executed (i.e.~a background thread)
	\item {\bfseries Line 11} (\code{doOnError}): to handle errors that might occur while processing the items
	\item {\bfseries Line 12} (\code{observeOn}): to specify in which thread the following operations should be executed (i.e.~main thread)
	\item {\bfseries Line 13} (\code{doOnCompleted}): to execute an action after the data processing has completed (i.e.~updating the UI)
\end{itemize}

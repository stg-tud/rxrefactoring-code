...
public class InstantiationCollector implements IWorker<Void, InstantiationCollector> {
	...
	// List of methods which, if set, define which methods can be implemented by a subclass for that
	// subclass to be stored in subclassDeclarations. If the subclass declares any additional methods
	// or fields, or the parameter types are not valid, it is not stored.
	// For instance, Arrays.asList("doInBackground", "onProgressUpdate", "onPostExecute");
	...
	/**
	 * Checks if a class instance creation of method invocation is a strict child (is not one itself) of an assignment, 
	 * a method invocation (as callee or argument) or a class instance creation.
	 */
	...
	/**
	 * Checks if the class only implements the allowed methods and declares no additional fields. The name
	 * of the methods are in {@link InstantiationCollector#implementableSubclassMethods}, and the parameter types
	 * must match those of a superclass declaration.
	 */

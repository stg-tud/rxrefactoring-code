...
public final class JavaVisitor {
	...
	public List<ASTNode> visitExtendedModifier(IExtendedModifier modifier) {
		if (modifier instanceof Annotation) {
			...
			return throwIncompatibleJavaException("visitExtendedModifier",
					(Class<? extends ASTNode>) modifier.getClass());
		}
	}
	 ...
	 * @return A list of all nodes that were retrieved while visiting the objects.
	 */
	...
	/**
	 * Throws a RuntimeException informing the user that their Java version is not
	 * compatible with this JavaVisitor version.
	 * 
	...
	private List<ASTNode> throwIncompatibleJavaException(String method, Class<? extends ASTNode> clazz) {
		throw new RuntimeException(JavaVisitor.class.getSimpleName() + " is not compatible with your Java version: "
				+ method + "() has to be updated, " + clazz.getName() + " could not be handled.");
	}
}

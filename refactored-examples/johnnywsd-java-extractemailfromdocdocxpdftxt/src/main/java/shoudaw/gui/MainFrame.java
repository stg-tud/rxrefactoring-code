...
public class MainFrame extends JFrame implements PropertyChangeListener {
	...
	private Subscription asyncTask;
	...
	// RxRefactoring: create observable instead of SwingWorker
	private Observable<Void> createRxObservable()
	{
		return Observable
				.fromCallable(() -> doInBackground())
	...
	// RxRefactoring: method can be private
	private Void doInBackground() throws Exception {
		String outputStr = FileChooser.saveFile();
		//			System.out.println(outputStr);

		progressMonitor.setProgress(0); // RxRefactoring: replace setProgress methods for a ProgressMonitor.setProgress
		...
		for(int i=0;i<filesString.length;i++){
			...
			progressMonitor.setProgress((int)((double)i/filesString.length *100)); // RxRefactoring: replace setProgress methods for a ProgressMonitor.setProgress
		}
...
//				task.addPropertyChangeListener(MainFrame.this); // RxRefactoring: remove the property change listener because this class implements its.
				asyncTask = createRxObservable().subscribe(); // RxRefactoring: cache the subscriber in field to be able to check if it is still subscribed or not
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if ("progress" == evt.getPropertyName() ) {
			...
			if (progressMonitor.isCanceled() || asyncTask.isUnsubscribed()) {
				Toolkit.getDefaultToolkit().beep();

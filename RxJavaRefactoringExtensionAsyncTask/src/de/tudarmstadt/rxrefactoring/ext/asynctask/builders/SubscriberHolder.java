...
/**
 * Description: Holds the necessary information to create Subscribers as
 * text<br>
...
public class SubscriberHolder {
	...
	/**
	 * Initializes a {@link Subscriber} with the action
	 * {@link Subscriber#onNext(Object)}. The methods
	 * {@link Subscriber#onCompleted()} and {@link Subscriber#onError(Throwable)}
	 * are empty.
	 *
	 *
	 * @param icuName
	...
	/**
	 * Creates a string corresponding to the method declaration that creates a
	 * subscriber based on the arguments passed to the constructor<br>
	 *
	 * The first name will be getRxUpdateSubscriber<br>
	 * The next names will be enumerated. i.e: getRxUpdateSubscriber1,
	 * getRxUpdateSubscriber2, etc.
	 * 
	 * @return the method as string
	 */
	public String getGetMethodDeclaration() {
		return "private Subscriber<" + type + "> getRxUpdateSubscriber" + getNumber() + "() " + "{ return "
				+ classInstantiation + "}";
	}
	 ...
	 * @param arguments
	 *            arguments
	 * @return the statement in charge of generating the next emission of the
	 *         subscriber
	 */
	...
	public String getSubscriberDeclaration() {
		return "final Subscriber<" + type + "> rxUpdateSubscriber" + getNumber() + " = getRxUpdateSubscriber"
				+ getNumber() + "();";
	}
	...
	public String getSubscribeString() {
		return "new ComplexRxObservable" + getNumber() + "().getAsyncObservable().subscribe();";
	}
